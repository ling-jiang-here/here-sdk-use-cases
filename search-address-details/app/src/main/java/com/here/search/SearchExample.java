...
public class SearchExample {
    ...
    public SearchExample(Context context, MapView mapView) {
        ...
        camera.lookAt(new GeoCoordinates(37.540840,-122.061478), mapMeasureZoom);
...
//    }
        ...
        public void onSuggestCompleted(@Nullable SearchError searchError, @Nullable List<Suggestion> list) {
            ...
            for (Suggestion autosuggestResult : list) {
                ...
                if (place != null) {
                    ...
                    fetchAddressDetails(place.getId());
                }
    ...
    private void fetchAddressDetails(String placeCategoryId) {
        ...
        searchEngine.searchByPlaceId(new PlaceIdQuery(placeCategoryId), languageCode, new PlaceIdSearchCallback() {
            ...
            public void onPlaceIdSearchCompleted(@Nullable SearchError searchError, @Nullable Place place) {
                ...
                Log.d(LOG_TAG, "Title: " + Objects.requireNonNull(place).getTitle());
                Log.d(LOG_TAG, "Title: " + place.getAddress().addressText);
                Log.d(LOG_TAG, "Country: " + placeAddress.country);
    ...
    private void autoSuggestExample() {
        ...
        searchEngine.suggestByText(
                new TextQuery("s gate", // User typed "s gate".
                        queryArea),
                searchOptions,
                autosuggestCallback);

        searchEngine.suggestByText(
                new TextQuery("s gate road", // User typed "s gate road".
                        queryArea),
                searchOptions,
                autosuggestCallback);

        searchEngine.suggestByText(
                new TextQuery("s gate road danville", // User typed "s gate road danville".
                        queryArea),
                searchOptions,
                autosuggestCallback);
    }
