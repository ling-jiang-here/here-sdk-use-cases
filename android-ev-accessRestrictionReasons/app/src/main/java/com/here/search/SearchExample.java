...
import com.here.sdk.search.EVChargingPool;
...
public class SearchExample {
    ...
    private final boolean isDeviceConnected = true;
...
//        camera.lookAt(new GeoCoordinates(52.520798, 13.409408), mapMeasureZoom);
        // Look at Lucid Motor US HQ
        camera.lookAt(new GeoCoordinates(37.54124, -122.06184), mapMeasureZoom);
...
//        searchInViewport(searchTerm);
        ...
        Toast.makeText(context, "Search for categories...", Toast.LENGTH_LONG).show();
        searchForCategories();
    }
...
//        GeoCoordinates geoCoordinates = new GeoCoordinates(52.53086, 13.38469);
        // Use Lucid Motor US HQ
        GeoCoordinates geoCoordinates = new GeoCoordinates(37.54124, -122.06184);
        double distanceInMeters = 1000 * 7;
        MapMeasure mapMeasureZoom = new MapMeasure(MapMeasure.Kind.DISTANCE_IN_METERS, distanceInMeters);
        camera.lookAt(geoCoordinates, mapMeasureZoom);

//        String queryString = "Invalidenstraße 116, Berlin";
        String queryString = "Lucid Motors US Headquarters";
    ...
    private void searchForCategories() {
        ...
        categoryList.add(new PlaceCategory(PlaceCategory.BUSINESS_AND_SERVICES_EV_CHARGING_STATION));
...
//        CategoryQuery.Area queryArea = new CategoryQuery.Area(new GeoCoordinates(52.520798, 13.409408));
        // Set search area to be around the headquarters of Lucid Motor in US
        CategoryQuery.Area queryArea = new CategoryQuery.Area(new GeoCoordinates(37.54124, -122.06184));
        ...
        offlineSearchEngine.searchByCategory(categoryQuery, searchOptions, new SearchCallback() {
            ...
            public void onSearchCompleted(SearchError searchError, List<Place> list) {
                ...
                // If error is null, list is guaranteed to be not empty.
                showDialog("Search results: " + list.size(), "See log for details.");

                for (Place searchResult : list) {
                    String addressText = searchResult.getAddress().addressText;
                    EVChargingPool evChargingPool = searchResult.getDetails().evChargingPool;
                    if (evChargingPool != null) {
                        List<EVAccessRestrictionReason> accessRestrictionReasons = evChargingPool.accessRestrictionReasons;
                        for (EVAccessRestrictionReason accessRestrictionReason : accessRestrictionReasons) {
                            if (addressText.contains("Tesla")) {
                                Log.d("accessRestrictionReason", addressText + " " + accessRestrictionReason.toString());
                            }
                        }
                    }
                }
